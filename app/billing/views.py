...
class WebhookReceiverView(APIView):
    ...
    def post(self, request):
        ...
        primary_signature = hmac.new(primary_secret, raw_payload, hashlib.sha256).hexdigest()

        # Check primary key first (Constant-time comparison prevents timing attacks)
        if not hmac.compare_digest(incoming_signature, primary_signature):
            # Fallback to secondary key if primary fails
            secondary_signature = hmac.new(secondary_secret, raw_payload, hashlib.sha256).hexdigest()
...
class GalleryUploadView(APIView):
    ...
    def post(self, request):
        ...
        if uploaded_file:
            ...
            is_zip = file_header.startswith(b'PK\x03\x04')

            if not is_valid_image and not is_zip:
